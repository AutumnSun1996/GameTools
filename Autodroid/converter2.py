...
def get_full_board(shot, multi_x, multi_y):
    ...
    for x in range(multi_x):
        for y in range(multi_y):
            full_board[bh*y:bh*(y+1), bw*x:bw*(x+1), :] = pixels2board(pixels[22*y:22*(y+1), 37*x:37*(x+1), :])
    return full_board
...
if __name__ == "__main__":
    ...
    video_out = cv.VideoWriter(name_tmp, apiPreference=cv.CAP_FFMPEG, fourcc=cv.VideoWriter_fourcc(*"mp4v"), fps=fps, frameSize=(w, h))
    ...
    name_out = "./PlayerSimulate/" + os.path.splitext(os.path.basename(name_in))[0] + "-AzurLanePlayer2.mp4"
    print("Combine to {}".format(name_out))
    scale_w = int(base.w / 4)
    scale_h = int(in_h * scale_w / in_w)
    os.system('ffmpeg -y -i "{0}" -i "{1}" -filter_complex "[0]scale=w={2}:h={3}[s];[1][s]overlay=x=main_w-overlay_w:y=main_h-overlay_h" -c:a aac -c:v h264 "{4}"'.format(name_in, name_tmp, scale_w, scale_h, name_out))