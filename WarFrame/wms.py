...
def set_message(df):
    ...
    if name.endswith(" Set"):
        ...
        name = "[" + name + "]"
    set_clipboard_text("/w {1.name} Hi! I want to buy: {0} for {1[platinum]} platinum.".format(name, df.iloc[0]))
...
def orders2df(info):
    ...
    } for item in info['payload']['orders'] if order_filter(item)]
...
def filt_orders(df, order_type="sell", rank=None, copy=True):
    sell_orders = df[(df['order_type'] == order_type) & (df['user_status'] == 'ingame') & df['visible']]
    try:
