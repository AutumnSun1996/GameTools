...
class FightMap(AzurLaneControl):
    ...
    def locate_target(self, target, image):
        ...
        for anchor in self.anchors:
            diff, anchor_pos = get_match(image, anchor["ImageData"])
    ...
    def click_at_map(self, target, repeat=0):
        ...
        if not self.resource_in_screen("迎击", image):
        ...
        target_pos = self.locate_target(target, image)
        if not target_pos:
